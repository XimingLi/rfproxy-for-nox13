...
class RFServer(RFProtocolFactory, IPC.IPCMessageProcessor):
	...
	self.log.info("rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr" )
	    ...
	    if entry is not None and entry.get_status() == RFENTRY_IDLE_VM_PORT:
	        return
	    else:
    ...
    def register_dp_port(self, ct_id, dp_id, dp_port):
        stop = self.config_dp(ct_id, dp_id, dp_port)
        if stop:
            return
    ...
    def config_dp(self, ct_id, dp_id, dp_port):
        ...
        return is_rfvs(dp_id) or (dp_port == 10000)
    ...
    def map_port(self, vm_id, vm_port, vs_id, vs_port):
        entry = self.rftable.get_entry_by_vm_port(vm_id, vm_port)
        if entry is not None:
	    if entry.get_status() == RFENTRY_ASSOCIATED or entry.get_status() == RFENTRY_ACTIVE:
