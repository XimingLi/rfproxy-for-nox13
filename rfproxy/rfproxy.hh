...
class Table {
	...
	void get_dp_to_vs_map_num() {
		
		lg.dbg("dp_to_vs map has %d elements",dp_to_vs.size());
	}

	int get_vs_to_dp_map_num() {

		lg.dbg("vs_to_dp map has %d elements",vs_to_dp.size());
	}
	void list_dp_to_vs() {

		map<PORT, PORT>::iterator it;
		for(it = dp_to_vs.begin(); it != dp_to_vs.end(); it++) {

		lg.dbg("dp_id = 0x%"PRIx64", dp_port = %d, vs_id = 0x%"PRIx64", vs_port = %d",it->first.first, it->first.second, it->second.first, it->second.second);
		}			
	}
	void list_vs_to_dp() {

		map<PORT, PORT>::iterator it;
		for(it = vs_to_dp.begin(); it != vs_to_dp.end(); it++) {

		lg.dbg("vs_id = 0x%"PRIx64", vs_port = %d, dp_id = 0x%"PRIx64", dp_port = %d",it->first.first, it->first.second, it->second.first, it->second.second);
		}
	}
	...
	void delete_vs_port(uint64_t vs_id, uint32_t vs_port) {
            map<PORT, PORT>::iterator it = dp_to_vs.begin();
            while (it != dp_to_vs.end()) {
                if ((*it).second.first == vs_id && (*it).second.second == vs_port)
                    dp_to_vs.erase(it++);
                else
                    ++it;
            }
            ...
            it = vs_to_dp.begin();
            while (it != vs_to_dp.end()) {
                if ((*it).first.first == vs_id && (*it).first.second == vs_port)
                    vs_to_dp.erase(it++);
                else
                    ++it;
            }
...
class rfproxy : public Component, private IPCMessageProcessor
{
	...
	Disposition on_port_status_in(const Event& e);
	Disposition on_port_description_in(const Event& e);
